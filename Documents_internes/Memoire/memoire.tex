...
\begin{itemize}
    ...
    \item La seconde méthode est une variante visant à réduire la taille du vecteur d'objets et est défini par le Space-Saving Algorithm \cite{SpaceSaving}. On choisit un vecteur de la taille du nombre de noeuds du réseau qui contient des structures de la forme \textit{( identifiant de l'objet ; nombre de requête )}. Soit $n$ le nombre de noeuds dans le réseau, l'algorithme permet de connaitre les $n$ objets les plus populaires. \newline
          ...
          Si $o$ n'est pas présent et que le vecteur est plein, on cherche l'endroit qui contient l'objet le moins populaire du vecteur et on le remplace par $o$. Cependant, on garde la popularité de l'ancien objet et on l'incrémente de 1. \newline
...
bar height = 0.8,
progress label text={},
bar/.append style={fill=green!10},
bar incomplete/.append style={fill=red!10}
...
\ganttbar[progress=20]{B1}{2015-03-07}{2015-03-14} \\ %Système d'assignation d'une requête
\ganttbar{B2}{2015-03-07}{2015-03-14} \\ %Communication charge file d'attente
\ganttbar[progress=10]{B3}{2015-03-15}{2015-03-24} \\ %Algorithme SLVO
\ganttbar[progress=10]{B4}{2015-03-15}{2015-03-24} \\ %Algorithme AvergeDegree
\ganttbar[progress=25]{BT}{2015-03-25}{2015-03-29} \\ %Tests

\ganttgroup{Gestion de la réplication (C)}{2015-02-16}{2015-03-07} \\
\ganttbar[progress=90]{C1}{2015-02-16}{2015-02-19} \\ %Créer une nouvelle stratégie de réplication
...
\ganttbar[progress=0]{D1}{2015-03-08}{2015-03-15} \\ %Calcul de la popularité pour un objet
\ganttbar[progress=0]{D2}{2015-03-16}{2015-03-20} \\ %Implémentation de SpaceSaving
\ganttbar[progress=0]{D3}{2015-03-21}{2015-03-25} \\ %Synchronisation des noeuds
\ganttbar[progress=0]{DT}{2015-03-26}{2015-03-29} \\ %Tests
...
\ganttbar[progress=10]{F1}{2015-03-10}{2015-03-15} \\ %Prise en main Graphite
\ganttbar{F2}{2015-03-16}{2015-03-21} \\ %Ajout metrics
\ganttbar[progress=0]{F3}{2015-03-22}{2015-03-28} \\ %Paramétrage

\ganttbar[progress=80]{T}{2015-04-01}{2015-04-04} \\
...
\paragraph{} Les classes s'occupant de collecter les mesures se situent dans le package \path{org.apache.cassandra.metrics}. Chaque classe de Metrics est chargée de collecter des statistiques sur certaines classes de Cassandra, comme par exemple sur la classe \class{Keyspace} vue précédemment dont les données sont collectées par la classe \class{KeyspaceMetrics}.
...
\paragraph{} Pour communiquer avec la base de données, nous utilisons un \textit{pilote}, programme informatique permettant à un programme d'interagir avec un autre programme.
...
Des jeux de requêtes simples, où l'on ne sélectionne qu'une seule ligne par requête et des jeux de grosses requêtes où l'on sélectionne plusieurs lignes par requêtes.
Des sélections de lignes suivant une loi aléatoire uniforme ou distribuée, pour tester le mécanisme de popularité.

\paragraph{} Pour cela, nous avons décidé de créer un générateur de requêtes pour chaque type de jeux de requêtes souhaité.
On appelle générateur de requêtes un objet de type héritant de la classe \class{QueriesFactory}. L'application a été conçue de manière à ce qu'il soit facile d'implémenter un nouveau générateur de requêtes.

\paragraph{} Créons le générateur de requêtes \class{RandomFactory} dont la probabilité de sélectionner une table lors d'une simulation ou une ligne lors d'une requête suit une loi de distribution uniforme.

\begin{figure}[h]
	\centering
		\includegraphics[width=10cm]{images/client/requete.png}
	\caption{Générateur de requêtes \class{RandomFactory} \label{fig:uml_generequete}}
\end{figure}

\paragraph{} Sur la figure~\ref{fig:uml_generequete}, nous remarquons que la classe \class{QueriesFactory} possède deux méthodes abstraites, \texttt{askForConfiguration} et \texttt{queriesFactory}.
L'attribut \texttt{column\_target} est le nom, dans la base de données, de la colonne \texttt{key}.
Dans un premier temps, concentrons nous sur \texttt{askForConfiguration}.


\paragraph{}
Pour exécuter un jeu de requêtes sur Cassandra, il faut utiliser la commande :
\shellcmd{queries <qf> <s> <ns> <nq>}
Avec les arguments :
\begin{itemize}
 ...
 \item \texttt{<ns>} Le nombre de simulations
 \item \texttt{<nq>} Le nombre de requêtes par simulation
\end{itemize}

\paragraph{} Pour exécuter un jeu de requêtes généré par \class{RandomFactory}, il faut donc taper la commande :
\shellcmd{queries RandomFactory <s> <ns> <nq>}
L'objet \class{RandomFactory} est crée dynamiquement, le constructeur appelé doit contenir quatre arguments ( \class{Connection}, \class{Long}, \class{Long}, \class{Long}).
Ainsi, tous les attributs de \class{QueriesFactory} sont initialisés. 
Si on souhaite initialiser les attributs de \class{RandomFactory} avec une valeur choisie par l'utilisateur, il faut utiliser la méthode \texttt{askForConfiguration}.
On peut ainsi demander à l'utilisateur la valeur qu'il souhaite pour l'attribut \texttt{column\_target} avec le code suivant :
...
\end{lstlisting}
\paragraph{} \texttt{Interactor.basicInput("Target column : ")} affichant le message \texttt{Target column :} dans la console et retournant la chaîne de caractères saisie par l'utilisateur.

\paragraph{} Dans un second temps, nous nous intéressons à la méthode \texttt{queriesfactory}. C'est dans cette méthode que nous implémentons le générateur de requêtes.
Pour \class{RandomFactory}, il s'agit d'implémenter l'algorithme suivant :
...
\paragraph{} Notre générateur de requêtes est opérationnel, une fois instancié, il ne reste plus qu'à exécuter la méthode \texttt{run}. Le temps d'exécution est chronométré puis affiché dans la console.

\paragraph{Fichier de log} Pour suivre l'évolution du jeu de requêtes, il est possible d'écrire un fichier de log. Pour enregistrer un message, il faut utiliser la méthode \texttt{addLog}.
L'écriture se fait à la fin de la génération et de l'exécution du jeu de requêtes pour éviter de fausser le chronomètre.


\subsubsection*{Problèmes et limites rencontrés}

\paragraph{} Pour générer une requête et sélectionner une ligne existante, on doit pouvoir connaître le nombre de lignes disponibles dans chaque table.
La base de données n'étant pas relationnelle, dénombrer les lignes peut prendre énormément de temps au point que Cassandra renvoie une erreur de type \texttt{Timeout}.

\paragraph{} Malheureusement pour effectuer une requête dans un keyspace, celui-ci doit être défini correctement, sinon l'exécution d'une requête enverra directement une exception.
Pour cela, lors de la création d'un objet \class{KeySpace}, la première requête que nous effectuons consiste à compter le nombre de ligne de chaque table.

\paragraph{} Si le keyspace est mal défini, lors du dénombrement des lignes, la requête échoue et le contructeur de \class{KeySpace} envoie une \class{UnavailableKeySpaceException}. 
Mais lorsque le dénombrement échoue car les données sont trop volumineuses, le constructeur envoie la même exception. Nous ne sommes donc pas en mesure de différencier un keyspace mal défini d'un keyspace bien défini mais
ayant une table contenant des données volumineuses.

\subsubsection*{Travail restant à réaliser}

\paragraph{} Le travail restant à réaliser concerne le problème exposé précédemment. Les pistes à explorer sont un dénombrement différent du nombre de lignes, une liaison entre générateur de données et générateur de requêtes,
la possibilité de rendre optionnel le décompte du nombre de lignes.

\subsubsection{Compilation et Tests}

\paragraph{} Pour compiler l'application cliente, il faut se placer dans le répertoire contenant le \class{pom.xml} (donc \texttt{Client/client}) et exécuter la commande
\shellcmd{mvn package}
L'exécutable \texttt{client.jar} est alors disponible dans le repertoire \class{target}.
Pour l'exécuter,
\shellcmd{java -jar client.jar}


\paragraph{} Les tests de l'application cliente ne sont pas complets. Tout d'abord, les objets ne dépendant pas du pilote, où ne faisant pas appelle à des méthodes solicitant le pilote sont testés avec JUnit lors de la compilation de l'application.

\paragraph{} Les autres objets ont été testés au fur et à mesure du développement de l'application, notamment grâce à la levée des exceptions, et en testant plusieurs saisies.
Pour tester la méthode de connexion au cluster par exemple, nous avons saisi les valeurs suivante, sachant que le cluster se trouve à 127.0.0.1 :
\begin{itemize}
  \item localhost : fonctionne
  \item 127.0.0.1 : fonctionne
  \item 8.8.8.8 : Timeout unreachable host
  \item coucou : Name or service not known
  \item 300.452.1 : Name or service not known
\end{itemize}

\paragraph{} L'automatisation de ces tests demande la présence d'une base de données Cassandra accessible (en local par exemple). En raison du manque de temps, nous n'avons pas pu les mettre en place.
...
\paragraph{}Nous tenions à remercier Frédéric Lalanne pour son aide au travers de la \textit{Jungle} de Cassandra, Noel Gillet et Nicolas Hanusse pour leur suivi et leurs conseils sur le projet.
...
\begin{figure}[p]
	\centering
		\include{images/architecture/stages}
