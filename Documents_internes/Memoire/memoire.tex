...
\paragraph{} Pour cela, nous avons décidé de créer un générateur de requête pour chaque type de jeux de requête souhaité.
On appelle générateur de requête un objet de type héritant de la classe \class{QueriesFactory}. L'application a été concue de manière à ce qu'il soit facile d'implémenter un nouveau générateur de requêtes.

\paragraph{} Créons le générateur de requête \class{RandomFactory} dont la probabilité de selectionner une table lors d'une simulation ou une ligne lors d'une requête suit une loi de distribution uniforme.
...
\paragraph{} Pour exécuter un jeu de requête généré par \class{RandomFactory}, il faut donc taper la commande :
...
\paragraph{} Malheureusement pour effectuer une requête dans un keyspace, celui-ci doit être défini correctement, sinon l'exécution d'une requête enverra direcetement une exception.
