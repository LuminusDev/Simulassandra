...
\paragraph{} Pour cela, nous avons décidé de créer un générateur de requêtes pour chaque type de jeux de requête souhaité.
On appelle générateur de requêtes un objet de type héritant de la classe \class{QueriesFactory}. L'application a été concue de manière à ce qu'il soit facile d'implémenter un nouveau générateur de requêtes.

\paragraph{} Créons le générateur de requêtes \class{RandomFactory} dont la probabilité de selectionner une table lors d'une simulation ou une ligne lors d'une requête suit une loi de distribution uniforme.

\begin{figure}[h]
	\centering
		\includegraphics[width=10cm]{images/client/requete.png}
	\caption{Générateur de requête \class{RandomFactory} \label{fig:uml_generequete}}
\end{figure}
...
L'objet \class{RandomFactory} est crée dynamiquement, le constructeur appelé doit contenir quatres arguments ( \class{Connection}, \class{Long}, \class{Long}, \class{Long}).
...
\end{lstlisting}
\paragraph{} \texttt{Interactor.basicInput("Target column : ")} affichant le message \texttt{Target column :} dans la console et retournant la chaîne de caractère saisie par l'utilisateur.

\paragraph{} Dans un second temps, interessons-nous à la méthode \texttt{queriesfactory}. C'est dans cette méthode que nous implémentons le générateur de requêtes.
...
\paragraph{} Notre générateur de requêtes est opérationnel, une fois instancié, il ne reste plus qu'à exécuter la méthode \texttt{run}. Le temps d'exécution est chonométré puis affiché dans la console.

\paragraph{Fichier de log} Pour suivre l'évolution du jeu de requête, il est possible d'écrire un fichier de log. Pour enregistrer un message, il faut utiliser la méthode \texttt{addLog}.
...
La base de données n'étant pas relationnelle, dénombrer les lignes peut prendre énormément de temps au point que Cassandra envoie une erreur de type \texttt{Time out}.
...
Pour cela, lors de la création d'un objet \class{KeySpace}, la première requête que nous effectuons est compter le nombre de lignes de chaque table.
...
\paragraph{} Le travail restant à réaliser concerne le problème exposé précédemment. Les pistes à explorer sont un dénombrement différent du nombre de ligne, une liaison entre générateur de données et générateur de requêtes,
la possibilité de rendre optionnel le decompte du nombre de ligne.
