...
Il est possible de définir le nombre de copie d'un objet au sein d'un ensemble de noeud, appelé \textit{data center}. 
...
\paragraph{} Des protocoles d'affectation plus spécifiques peuvent être implémentés. Ils utilisent les données locales du noeud pour décider de l'affectation des requêtes.
...
Si l'utilisateur ne détient pas de suites de requêtes, il pourra demander à l'application d'en créer pour lui. 
...
Elle peut par exemple, compter le nombre de données sauvegardées, chercher si une donnée existe réellement, mais ne peut pas en modifier une.
...
\paragraph{} Il n'a pas encore été décidé de remettre un ou plusieurs livrables intermédiaire au client.
