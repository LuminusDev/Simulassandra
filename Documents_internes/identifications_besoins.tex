...
\paragraph{Données d'un noeud} Le client souhaite que chaque noeud contiennent les données \besoin{1}{1} suivantes :
\begin{itemize}
...
\paragraph{Mise à jour des données} Afin de connaître l'état du réseau de manière précise, les données doivent être mise à jour à chaque action \besoin{1}{1}.
Une mise à jour à donc lieu :
\begin{itemize}
 \item A l'arrivée d'une requête
 \item Lors de l'ajout d'un objet
\end{itemize}


\paragraph{Récupération de l'état du réseau} L'application doit permettre la permettre la description de l'état du réseau \besoin{1}{1}.
On souhaite connaître la charge des noeuds, le nombre de requêtes en attente, la popularité des objets.
...
\paragraph{} La calcul de la popularité nécessite l'implémentation de l'algorithme d'approximation Space-Saving Algorithm \cite{SpaceSaving}.
...
\paragraph{} Une fois le réseau crée, l'utilisateur peut créer une 
\paragraph{} Le client souhaite pouvoir :
\begin{itemize}
...
Pour cela, nous nous appuierons sur le protocole \texttt{Gossip} \cite{gossip}.
...
Pour cela, nous avons défini quelques normes pour que le projet puisse être repris.
