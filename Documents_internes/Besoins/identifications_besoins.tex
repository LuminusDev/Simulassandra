...
Echanger des informations entre les noeuds

REQUETE DE LECTURE
File d'attente des requêtes client
	- Ajouter / Traiter
Connaitre les informations des noeuds du réseau
Connaitre la stratégie de réplication
Identifier les noeuds responsables de l'objet
Message
	- Envoyer un message aux noeuds responsables pour traiter une requête de lecture
	- Recevoir le message
File d'attente des requêtes de lecture
	- ajouter
	- Traiter
	- Supprimer
Envoyer un message aux noeuds responsables pour supprimer une requête de lecture (identification requête)
	-> recevoir (a)
Renvoyer le resultat au noeud coordinateur
Transmettre le résultat du noeud coordinateur au client


REQUETE D'ECRITURE

Message
	- Envoyer un message aux noeuds responsables pour traiter une requête d'écriture
	- Recevoir le message
File d'attente des requêtes d'écriture
	- ajouter
	- Traiter
Renvoyer le resultat au noeud coordinateur
Transmettre le résultat du noeud coordinateur au client
...
\paragraph{Connexion à Cassandra}  \besoin{1}{1}

\paragraph{Récupérer le nom des tables contenues dans Cassandra} \besoin{1}{1} Afin de pouvoir générer des requêtes.
 

\subsubsection{Initialisation des données}

\paragraph{Créer un Keyspace} \besoin{1}

\paragraph{Importer des données} \besoin{1}
...
\paragraph{Générer un jeu de données pseudo-aléatoirement} \besoin{2}{1} Il s'agit de coder une fonction $f$, qui aura pour ensemble des antecedants des suites de caractères alpha-numériques (par exemple: 5832fg4gh52 ) et pour image un jeu de données.

\paragraph{Importer un jeu de requêtes} \besoin{1}{1} L'utilisateur peut importer son propre jeu de requêtes.
