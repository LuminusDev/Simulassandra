...
\paragraph{} Un \textit{objet} est composé d'une clé d'identification appelé \textit{token} et d'un ensemble de \texit{données}, codage (une représentation sous forme binaire), propre au système de base de données, d'une information quelconque.
...
\paragraph{} Le but est de comparer l'efficacité des protocoles de réaffectation des requêtes de lectures. Pour cela, on doit être capable de visualiser les statistiques de fonctionnement de la base de données distribuée.
